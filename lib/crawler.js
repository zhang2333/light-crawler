...
// load rule
Crawler.prototype.loadRule = function loadRule(rule, expand) {
    var self = this;
...
Crawler.prototype._insertRule = function _insertRule(rule) {
    rule.reg = rule.reg.replace(/\./g, '\\.').replace(/\*\*/g, '.*').replace(/<\.>/g, '.');
    rule.scrape = rule.scrape.bind(this);
    this._rules.push(rule);
}

Crawler.prototype.removeRule = function removeRule(ruleName) {
    ...
    function matchRule(result) {
        ...
        for (var r of self._rules) {
            if (new RegExp(r.reg).test(task.url)) {
                var matched = _.isFunction(r.match) ? (r.match(task) || false) : false;
                var matchAttrs = _.isMatch(task, _.omit(r, ['reg', 'scrape', 'expand', 'match', 'ruleName']));
        ...
        Promise.delay(10).then(function() {
            process.nextTick(function(result) {
                // scraping
                if (matchedRule) {
                    r.scrape(result, cheerio.load(result.body), r.expand);
                    self.doneCounter++;
                }
                self._drainAwait();
            }, result);
 ...
 * stop the Crawler
 */
Crawler.prototype.stop = function stop() {
    var self = this;
...
Crawler.prototype.uniqTasks = function uniqTasks() {
    var self = this;
