...
Crawler.prototype.loadRule = function loadRule(rule, expand) {
    ...
    rule.reg = trimReg(rule.reg);
    self._rules.push(rule);

    return self;
}

function trimReg(reg) {
    return reg.replace(/\./g, '\\.').replace(/\*\*/g, '.*').replace(/<\.>/g, '.');
}
    ...
    function matchRule(result) {
        ...
        for (var r of self._rules) {
            if (new RegExp(r.reg).test(task.url)) {
                ...
                var matchAttrs = _.isMatch(task, _.omit(r, ['reg', 'scrape', 'expand', 'match']));
