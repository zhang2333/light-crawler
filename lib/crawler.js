    ...
    self.on('pollTask', function () {
        self.p = Promise.map(self._tasks, function (task) {
            ...
            return Promise.delay(Math.abs(interval)).then(function () {
                // cache tasks
                if (self.settings.skipRepetitiveTask === true) {
            ...
            .finally(function () {
                if (self.paused !== true) {
                    if (self._tasks.length !== 0) {
                        self.emit('pollTask');
                    } else if (self._readyTasks.length !== 0) {
...
Crawler.prototype.addTasks = function addTasks(taskUrls, props) {
    ...
    if (Array.isArray(taskUrls)) {
        ...
        if (self.settings.skipRepetitiveTask === true && new RegExp(taskUrls).test(self._taskCache)) {
            return self;
        }
...
Crawler.prototype._crawl = function _crawl(task) {
    ...
    // call beforeCrawlTask
    if (self.beforeCrawl) {
        self.beforeCrawl(task);
    }
