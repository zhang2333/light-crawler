...
Crawler.prototype._init = function _init(opts) {
	...
	self.q = async.queue(function (task, done) {
		var crawl = function crawl() {
			...
			// log current
			self.log(util.format('Current No.%s: %s', self.taskCounter + 1, task.url));
	...
	self.q.drain = function () {
		self.log(util.format('[Crawler Finished] Total: %s.Success: %s.Fail: %s.',
