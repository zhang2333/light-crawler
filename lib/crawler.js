	...
	self.q = async.queue(function (task, done) {
		var crawl = function crawl() {
			...
			self._crawl(task, function (result) {
				...
				if (!flag) {
					self.errLog = self.errLog.concat(util.format('[No Rule Handle][No.%s]%s\n', result.id, result.url));
					done();
				}
			});
		};
 ...
 */
Crawler.prototype.addTasks = function addTask(taskUrls, props) {
	var self = this;
...
Crawler.prototype._crawl = function _crawl(url, callback) {
	...
	var opts = _.assign(self.requestOpts, { url: url });
	if (opts.proxy == '') {
		delete opts.proxy;
	}
	...
	var func = function func(cb) {
		request(opts, function (e, r, body) {
			if (!e && r.statusCode == 200) {
				cb(null, { id: id, url: url, body: body });
			} else {
